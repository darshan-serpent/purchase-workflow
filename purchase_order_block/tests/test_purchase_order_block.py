...
class TestPurchaseOrderBlock(TransactionCase):

    def setUp(self):
        super(TestPurchaseOrderBlock, self).setUp()
        self.users_obj = self.env['res.users']
        self.po_obj = self.env['purchase.order']
        self.po_block_obj = self.env['purchase.block.reason']
        # company
    ...
    def _create_block_reason(self):
        self.po_block_reason = self.po_block_obj.create({
    ...
    def _create_purchase(self, line_products):
        ...
        purchase = self.po_obj.create({
            ...
            'po_block_id': self.po_block_reason.id,
            'order_line': lines,
